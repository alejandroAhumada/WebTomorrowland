...
type ResearchScope = 'all' | 'plans' | 'events'
...
function parseScope(value: string): ResearchScope {
  if (value === 'all' || value === 'plans' || value === 'events') return value
  throw new Error('RESEARCH_SCOPE debe ser all, plans o events.')
}
...
async function readState(path: string): Promise<ResearchState> {
  try {
    ...
    return parsed && typeof parsed.sources === 'object' ? parsed : { sources: {}, eventSources: {} }
  } catch { return { sources: {}, eventSources: {} } }
}
