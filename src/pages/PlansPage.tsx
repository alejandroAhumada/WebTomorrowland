...
export function PlansPage({ travelerCount }: { travelerCount: 1 | 2 }) {
  ...
  return <div className="page-container plans-page">
    ...
    {!loading && !error && <div className="plans-grid">{sortedPlans.map((plan) => <PlanCard key={plan.id} plan={plan} selected={isSelected(plan.id)} disabled={isFull} onToggle={() => toggle(plan.id)} />)}</div>}
    <ComparisonBar />
  </div>
}
