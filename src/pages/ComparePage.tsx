...
import { getPlanAccommodationInclusion, getPlanCatalogEntry, getPlanTierOptions } from '../models/planCatalog'
...
function buildRows(tiers: ReturnType<typeof useTicketTiers>['tiers']): ComparisonRow[] {
  return [
    ...
    { label: 'Alojamiento', essential: true, differenceKey: (plan) => `${getPlanAccommodationInclusion(plan)}:${plan.accommodation}`, render: (plan) => <><strong>{knowledgeLabel(getPlanAccommodationInclusion(plan), 'Incluido', 'No incluido')}</strong><small>{plan.accommodation}</small></> },
    ...
    { label: 'Alojamiento externo estimado', differenceKey: (plan) => String(getPlanAccommodationInclusion(plan)), render: (plan) => <BudgetCompareValue plan={plan} category="EXTERNAL_ACCOMMODATION" /> },
...
export function BudgetCompareValue({ plan, field, category }: { plan: TravelPlan; field?: 'total' | 'perPerson'; category?: BudgetCategory }) {
  ...
  if (field === 'total') return <span className="budget-compare">{budget.total ? `≈ ${formatMoney(budget.total)}` : budget.pendingReason === 'PLAN_PRICE' ? 'Pendiente del precio Tomorrowland' : 'Presupuesto incompleto'}</span>
  if (field === 'perPerson') return <span className="budget-compare emphasis">{budget.totalPerPerson ? `≈ ${formatMoney(budget.totalPerPerson)}` : budget.pendingReason === 'PLAN_PRICE' ? 'Pendiente' : 'Información insuficiente'}</span>
  const item = budget.items.find((candidate) => candidate.category === category)
  if (category === 'EXTERNAL_ACCOMMODATION' && budget.accommodationIncluded === true) return <span className="budget-compare positive">Incluido</span>
  if (category === 'EXTERNAL_ACCOMMODATION' && budget.accommodationIncluded === null) return <span className="budget-compare muted">No informado</span>
...
function BooleanValue({ value, trueLabel, falseLabel }: { value: boolean | null; trueLabel: string; falseLabel: string }) {
  if (value === null) return <span className="boolean-value"><CircleMinus aria-hidden="true" />No informado</span>
...
function knowledgeLabel(value: boolean | null, trueLabel: string, falseLabel: string): string { return value === true ? trueLabel : value === false ? falseLabel : 'No informado' }
