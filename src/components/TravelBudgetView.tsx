...
export function TravelBudgetBreakdown({ budget, loading = false }: { budget: TravelBudget; loading?: boolean }) {
  return <section className="budget-breakdown" aria-labelledby={`budget-${budget.planId}`}>
    ...
    })}{budget.accommodationIncluded === true && <li className="budget-included"><span className="budget-item-icon"><Hotel aria-hidden="true" /></span><div><strong>Alojamiento</strong><small>Ya está incluido en el plan Tomorrowland.</small></div><b>Incluido</b></li>}</ul>
