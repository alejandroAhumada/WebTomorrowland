...
test('smoke de Home, catálogo y rutas principales', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Alternativas para empezar a comparar' })).toBeVisible()
...
test('recomendaciones agrupan razones, distinguen métricas y excluyen Mi Viaje', async ({ page }) => {
  await page.setViewportSize({ width: 390, height: 844 })
  await page.goto('/')
  const section = page.locator('.recommendations-section')
  await section.getByRole('button', { name: '1 persona' }).click()
  await expect(section.getByText('Calculando alternativas', { exact: false })).toBeHidden({ timeout: 30_000 })
  const fullMadness = section.locator('.recommendation-card').filter({ hasText: 'Full Madness Pass · 1 persona' })
  await expect(fullMadness).toHaveCount(1)
  await expect(fullMadness.getByText('Viaje estimado más bajo')).toBeVisible()
  await expect(fullMadness.getByText('Precio Tomorrowland más bajo')).toBeVisible()
  await expect(fullMadness.getByText('Viaje estimado · WebTomorrowland')).toBeVisible()
  await expect(fullMadness.getByText('Precio Tomorrowland', { exact: true })).toBeVisible()
  await expect(fullMadness.locator('.price-badge')).toHaveCount(0)
  await expect(fullMadness.getByRole('button', { name: 'Ver plan' })).toBeVisible()

  await section.getByRole('button', { name: '2 personas' }).click()
  const derived = section.locator('.recommendation-card').filter({ hasText: '2 × Full Madness Pass' })
  await expect(derived).toContainText('2 entradas individuales · cálculo para 2 personas')
  await expect(derived).toContainText('R$ 3.160 por entrada · precio individual oficial')
  await expect(derived).not.toContainText('R$ 6.320 · precio oficial')
  await expect(section.getByRole('button', { name: 'Comparar estas alternativas' })).toBeVisible()
  await expectNoOverflow(page)

  await selectMyPlan(page, 'Easy Tent 2P')
  const alternatives = page.locator('.recommendations-section.secondary')
  await expect(alternatives.getByText('Calculando alternativas', { exact: false })).toBeHidden({ timeout: 30_000 })
  await expect(alternatives.getByRole('heading', { name: 'Easy Tent 2P' })).toHaveCount(0)
  await expect(alternatives.getByText('Mi plan')).toHaveCount(0)
})
